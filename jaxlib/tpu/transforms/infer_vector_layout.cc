...
#define NYI(msg)                            \
  ...
  LogicalResult inferBlock(
    ...
    for (Operation &any_op : block.without_terminator()) {
      ...
      if (!has_vector_io && any_op.getRegions().empty()) {
        ...
        CHECK_OP(static_cast<bool>(true_ty) == static_cast<bool>(false_ty),
                 "Only one side of arith is a vector?");
        if (true_ty) {
          CHECK_OP(true_ty.getElementTypeBitWidth() == kNativeBitwidth &&
        ...
        CHECK_OP(static_cast<bool>(in_ty) == static_cast<bool>(out_ty),
                 "Input and output are not both vectors?");
        if (in_ty) {
          CHECK_OP(in_ty.getElementTypeBitWidth() == 1 &&
        ...
        CHECK_OP(static_cast<bool>(lhs_ty) == static_cast<bool>(rhs_ty),
                 "Only one side of cmp is a vector?");
        if (lhs_ty) {
          CHECK_OP(lhs_ty.getElementTypeBitWidth() == kNativeBitwidth &&
  ...
  LogicalResult infer<arith::ConstantOp>(arith::ConstantOp op) {
    ...
    if (auto ty = dyn_cast<VectorType>(op.getType())) {
      ...
      CHECK_OP(ty.getElementType().isSignlessIntOrIndexOrFloat(),
               "expected scalar element type in vector");
      CHECK_OP(ty.getRank() > 0, "rank 0 vectors unsupported");
      CHECK_OP(elems, "expected vector constants to use DenseElementsAttr");
      ...
      if (elems.isSplat()) {
        ...
        CHECK_OP(ty.getElementTypeBitWidth() == kNativeBitwidth,
  ...
  LogicalResult infer<arith::ExtFOp>(arith::ExtFOp op) {
    ...
    CHECK_OP(src_ty.getElementTypeBitWidth() == 16 &&
    ...
    if (layout.implicit_dim() == ImplicitDim::kSecondMinor) {
      CHECK_OP(layout.tiling() == nativeTiling(16), "unsupported tiling");
      auto dst_layout = VectorLayout(32, layout.offsets(), default_tiling_,
                                     layout.implicit_dim());
  ...
  LogicalResult infer<arith::TruncFOp>(arith::TruncFOp op) {
    ...
    auto dst_ty = cast<VectorType>(op.getOut().getType());
    auto some_layout = getLayout(op.getIn());
    CHECK_OP(src_ty.getElementTypeBitWidth() == 32 &&
  ...
  LogicalResult infer<func::FuncOp>(func::FuncOp op) {
    ...
    return inferBlock(
        op.getBody().front(), [this](Operation *op) -> LogicalResult {
          CHECK_OP(isa<func::ReturnOp>(op), "Expected func.return terminator");
          for (Value o : op->getOperands()) {
            CHECK_OP(!isa<VectorType>(o.getType()),
  ...
  LogicalResult infer<scf::IfOp>(scf::IfOp op) {
    ...
    CHECK_OP(op->getNumOperands() == 1, "expected one operand");
    setInLayout(op, {kNoLayout});
    if (inferBlock(*op.thenBlock(), match_yield).failed()) {
      return failure();
    }
    auto then_yield = op.thenBlock()->getTerminator();
    CHECK_OP(then_yield->getOperandTypes() == op->getResultTypes(),
    ...
    CHECK_OP(else_yield->getOperandTypes() == op->getResultTypes(),
  ...
  LogicalResult infer<tpu::TraceOp>(tpu::TraceOp op) {
    ...
    CHECK_OP(op->getNumOperands() == 0, "expected no operands");
    CHECK_OP(op->getNumResults() == 0, "results unsupported");
  ...
  LogicalResult infer<tpu::IotaOp>(tpu::IotaOp op) {
    ...
    CHECK_OP(ty.getRank() >= 2, "iota rank below 2D unsupported");
  ...
  LogicalResult infer<vector::BroadcastOp>(vector::BroadcastOp op) {
    ...
    if (some_src_ty.isSignlessIntOrIndexOrFloat()) {
      CHECK_OP(some_src_ty.getIntOrFloatBitWidth() == kNativeBitwidth,
    ...
    if (auto src_ty = dyn_cast<VectorType>(some_src_ty)) {
      CHECK_OP(src_ty.getRank() >= 2, "source rank below 2D unsupported");
      CHECK_OP(res_ty.getRank() >= 2, "result rank below 2D unsupported");
      auto some_layout = getLayout(op.getSource());
      CHECK_OP(some_layout.has_value(), "missing vector layout");
      auto &layout = *some_layout;
      CHECK_OP(layout.implicit_dim() == ImplicitDim::kNone,
               "expected 2D layout");
  ...
  LogicalResult infer<vector::ContractionOp>(vector::ContractionOp op) {
    ...
    CHECK_OP(op.getIteratorTypes() == matmul_iterator_types, "Not a matmul");
    ...
    const auto matmul_indexing_maps_transposed = mlir::ArrayAttr::get(
        ...
        {AffineMapAttr::get(AffineMap::get(
         ...
         AffineMapAttr::get(AffineMap::get(
             3, 0, {getAffineDimExpr(0, ctx), getAffineDimExpr(1, ctx)},
             ctx))});
    CHECK_OP(op.getIndexingMaps() == matmul_indexing_maps ||
    ...
    CHECK_OP(res_ty, "only vector results supported");
    CHECK_OP(res_ty.getElementType().isF32(),
  ...
  LogicalResult infer<vector::LoadOp>(vector::LoadOp op) {
    ...
    for (Value v : op.getIndices()) {
      ...
      CHECK_OP(cst_op, "only constant indices are supported");
      indices.push_back(cast<IntegerAttr>(cst_op.getValue()).getInt());
    }
    for (int64_t i = 0; i < rank; ++i) {
      CHECK_OP(indices[i] + res_ty.getDimSize(i) <= src_ty.getDimSize(i),
    ...
    if (rank == 1) {
      CHECK_OP(tiling.size() == 1, "Expected 1D tiling in 1D loads");
      auto tile = tiling.front();
      CHECK_OP(tile % target_shape_[1] == 0, "Unsupported tiling for 1D load");
      ...
      CHECK_OP(tiling.size() == 2, "Expected 2D tiling in 2D+ loads");
      ...
      CHECK_OP(tiling[1] == target_shape_[1], "Unsupported tiling for 2d load");
  ...
  LogicalResult infer<vector::ExtractStridedSliceOp>(
    ...
    for (auto offset_attr : offsets) {
      ...
      CHECK_OP(off == 0, "Only zero-offset slices supported.");
    }
    sizes = sizes.take_back(2);
    CHECK_OP((sizes[0].cast<IntegerAttr>().getInt() % 8 == 0) &&
                 (sizes[1].cast<IntegerAttr>().getInt() % 128 == 0),
             "Only lane and sublane aligned slices allowed.");
    for (auto stride : strides) {
      CHECK_OP(stride.cast<IntegerAttr>().getInt() == 1,
  ...
  LogicalResult infer<vector::MultiDimReductionOp>(
    ...
    CHECK_OP(src_ty.getRank() == dst_ty.getRank() + 1,
    ...
    CHECK_OP(is_fully_replicated(acc_pad),
             "only constant accumulators supported");
    CHECK_OP(src_ty.getElementTypeBitWidth() == kNativeBitwidth,
             "only 32-bit reductions supported");
    auto some_src_layout = getLayout(op.getSource());
    CHECK_OP(some_src_layout, "missing vector layout");
    auto &src_layout = *some_src_layout;
    CHECK_OP(src_layout.implicit_dim() == ImplicitDim::kNone,
  ...
  LogicalResult infer<vector::ShapeCastOp>(vector::ShapeCastOp op) {
    ...
    if (layout.implicit_dim() == ImplicitDim::kNone) {
      ...
      CHECK_OP(src_ty.getRank() >= 2, "expected 2D+ operand with 2D layout");
  ...
  LogicalResult infer<vector::StoreOp>(vector::StoreOp op) {
    ...
    for (int64_t i = 0; i < rank; ++i) {
      CHECK_OP(indices[i] + store_ty.getDimSize(i) <= ref_ty.getDimSize(i),
    ...
    if (rank == 1) {
      CHECK_OP(tiling.size() == 1, "Expected 1D tiling in 1D store");
      auto tile = tiling.front();
      CHECK_OP(tile % target_shape_[1] == 0,
      ...
      CHECK_OP(tiling.size() == 2, "Expected 2D tiling in 2D+ store");
      std::array<std::optional<int64_t>, 2> offsets;
      const auto tile_indices = ArrayRef<int64_t>(indices).take_back(2);
  ...
  LogicalResult infer<vector::TransposeOp>(vector::TransposeOp op) {
    ...
    auto &layout = *some_layout;
    auto src_ty = op.getSourceVectorType();
    CHECK_OP(permutation_attrs.size() == src_ty.getRank(),
             "Transpose permutation has incorrect rank");
    if (layout.implicit_dim() == ImplicitDim::kNone) {
      CHECK_OP((layout.offsets() == LayoutOffsets{0, 0}),
               "Padded transposes unsupported");
      auto xlu_width = target_shape_[1];
      for (int64_t s : src_ty.getShape().take_back(2)) {
        CHECK_OP(s % xlu_width == 0, "Padded transposes unsupported");
      }
      for (auto attr : permutation_attrs.drop_back(2)) {
        CHECK_OP(cast<IntegerAttr>(attr).getInt() < src_ty.getRank() - 2,
  ...
  LogicalResult inferElementwise(Operation *op, bool check_bitwidth = true) {
    CHECK_OP(op->getNumResults() == 1, "only one result supported");
    CHECK_OP(op->getNumOperands() > 0,
    ...
    for (int64_t i = 0; i < op->getNumOperands(); ++i) {
      if (auto vty = dyn_cast<VectorType>(op->getOperand(i).getType())) {
        ...
        CHECK_OP(op->getOperand(i).getType().isSignlessIntOrIndexOrFloat(),
    ...
    if (auto out_vty = dyn_cast<VectorType>(op->getResult(0).getType())) {
      CHECK_OP(!check_bitwidth || bit_width == out_vty.getElementTypeBitWidth(),
