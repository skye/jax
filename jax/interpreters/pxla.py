...
def parallel_callable(fun: lu.WrappedFun,
  ...
  if config.omnistaging_enabled:
    ...
    if global_replica_shapes is not None:
      global_sharded_avals = [
          ShapedArray(shape, aval.dtype) if shape is not None else aval
          for shape, aval in safe_zip(global_replica_shapes, sharded_avals)]
    else:
      global_sharded_avals = sharded_avals
